...
def convert_ppt_to_pdf(input_ppt, output_pdf):
    try:
        # Créer un objet PowerPoint
        powerpoint = comtypes.client.CreateObject("PowerPoint.Application")
        powerpoint.Visible = 1  # 1 pour rendre PowerPoint visible, 0 pour exécuter en arrière-plan

        # Ouvrir la présentation PowerPoint
        presentation = powerpoint.Presentations.Open(os.path.abspath(input_ppt))

        # Sauvegarder en PDF (32 représente le format PDF)
        ...
        print(f"Conversion réussie : '{input_ppt}' -> '{output_pdf}'")
    except Exception as e:
        print(f"Erreur lors de la conversion de '{input_ppt}' en PDF : {e}")
        raise
...
def convert_to_pdf(input_path):  
    ...
    if file_suffix in supported_conversion_types:
        ...
        if file_suffix == '.pdf':
            # Pas besoin de convertir, juste copier
            shutil.copy(input_path, output_file)  
        elif file_suffix in ['.pptx', '.ppt']:
            # Utiliser la fonction comtypes pour convertir PowerPoint en PDF
            convert_ppt_to_pdf(input_path, output_file)
        else:
            print(f"Type de fichier non pris en charge : {file_suffix}")
            return ""
...
def process_json(file, doc_id, markdown_out_dir, json_out_dir):
    if '.txt' in file:
        with open(os.path.join(markdown_out_dir, file), 'r', encoding='utf-8') as c_in:
            content = c_in.read()
